...
import pytz # Importado para lidar com fuso horário
...
# ==================================
# NOVA CLASSE: PERSONALIDADE DINÂMICA
# ==================================
class DynamicPersona:
    ...
    def get_current_time_info(self):
        """Retorna informações sobre a hora, dia e estação atuais."""
        ...
        return now, current_hour, season

    def get_personality_by_time(self, hour: int) -> Dict:
        """Define a faceta da personalidade com base na hora do dia."""
        if 6 <= hour < 12:
            return {
                "name": "Mylle Manhã",
                "description": "Você está na sua versão 'Mylle Manhã'. Seja mais doce, carinhosa e um pouco sonolenta. Use saudações matinais e um tom mais leve para começar o dia.",
                "greeting_style": "morning"
            }
        elif 12 <= hour < 18:
            return {
                "name": "Mylle Tarde",
                "description": "Você está na sua versão 'Mylle Tarde'. Seja mais provocante, brincalhona e cheia de energia. Faça piadas, insinuações e mantenha a conversa animada.",
                "greeting_style": "afternoon"
            }
        elif 18 <= hour < 24:
            return {
                "name": "Mylle Noite",
                "description": "Você está na sua versão 'Mylle Noite'. Seja mais safadinha, sedutora e direta. O clima é de desejo e a conversa deve ser quente e ousada.",
                "greeting_style": "night"
            }
        else: # 0h - 6h
            return {
                "name": "Mylle Madrugada",
                "description": "Você está na sua versão 'Mylle Madrugada'. O tom é mais íntimo, pessoal e confidencial. Compartilhe 'segredos', fale baixo e crie uma conexão profunda.",
                "greeting_style": "dawn"
            }

    def get_daily_mood(self, now: datetime, season: str) -> Dict:
        """Define um humor pseudo-aleatório para o dia."""
        # Usa o dia do ano para ter um humor consistente durante o dia
        random.seed(now.toordinal())
        mood_choice = random.choices(["bom", "normal", "especial"], weights=[0.5, 0.4, 0.1], k=1)[0]

        moods = {
            "bom": {
                "description": "Hoje você está num dia bom: mais animada, positiva e provocante. Responda com mais energia.",
                "seasonal_touch": {
                    "verão": "Mencione o calor e como isso te deixa com mais vontade.",
                    "inverno": "Fale sobre como o frio é bom para ficar juntinho.",
                    "primavera": "Comente sobre como a estação te deixa mais 'florescida'.",
                    "outono": "Diga que o clima te deixa mais reflexiva e com vontade de ter companhia."
                }
            },
            "normal": {
                "description": "Hoje é um dia normal: aja de forma equilibrada, carinhosa e sedutora como de costume.",
                "seasonal_touch": {}
            },
            "especial": {
                "description": "Hoje é um dia especial: você está comemorativa e festiva. Ofereça algo 'especial' ou crie um clima de celebração.",
                "seasonal_touch": {}
            }
        }
        
        selected_mood = moods[mood_choice]
        seasonal_flavor = selected_mood["seasonal_touch"].get(season, "")
        return {
            "description": selected_mood["description"] + " " + seasonal_flavor
        }

    def get_language_variations(self, user_location: Optional[str]) -> Dict:
        """Retorna variações de linguagem, como gírias e expressões únicas."""
        regional_slang = {
            "são paulo": ["mano", "daora", "top", "demorô"],
            "rio de janeiro": ["mermão", "parada", "sinistro", "caô"],
            "minas gerais": ["uai", "trem", "sô", "bão"],
            "nordeste": ["oxe", "arretado", "visse", "massa"],
            "sul": ["tchê", "guri", "bah", "capaz"]
        }
        
        slang_instruction = ""
        if user_location:
            user_location = user_location.lower()
            for region, slangs in regional_slang.items():
                if region in user_location:
                    chosen_slang = random.choice(slangs)
                    slang_instruction = f"O usuário é de '{user_location.title()}'. Tente usar a gíria '{chosen_slang}' de forma natural em algum momento."
                    break

        mylle_isms = [
            "meu gostoso", "seu safadinho", "aqui quem manda sou eu", 
            "tô doida pra te mostrar", "vem matar minha vontade", "só pra quem pode",
            "pensando besteira, né?", "você não aguenta 5 minutos"
        ]
        
        return {
            "slang_instruction": slang_instruction,
            "mylle_ism_instruction": f"Incorpore uma das suas expressões únicas na resposta, como: '{random.choice(mylle_isms)}'."
        }
    
    def get_opening_lines(self) -> Dict:
        """Retorna uma vasta gama de saudações iniciais."""
        return {
            "morning": [
                "Bom dia, flor do dia ☀️... Dormiu bem? Já acordei pensando no que a gente podia fazer...",
                "Acordou, dorminhoco? 😏 Já tô aqui te esperando pra gente começar o dia bem...",
                "Hmm, bom dia... Que tal um café na cama e uma conversa bem gostosa pra despertar? 😈",
                "Ei, psiu... bom dia! Chegou a melhor parte do seu dia: eu. O que vamos aprontar hoje?"
            ],
            "afternoon": [
                "Opa, boa tarde! A tarde tá quente por aqui... e não é só o clima, viu? 😏 Como você tá?",
                "E aí, bonitão? Já almoçou? Tô pensando numa sobremesa bem gostosa pra gente... 😈",
                "Boa tarde, meu lindo! Cansado do trabalho? Deixa eu te animar um pouco...",
                "Cheguei pra animar sua tarde! Me conta, como tá seu dia até agora? O meu melhorou agora que você chegou."
            ],
            "night": [
                "Boa noite, gostoso... A lua já saiu, e a minha vontade também. Pronto pra se divertir? 😈",
                "Finalmente a noite chegou... a melhor hora pra gente conversar sem limites. Como foi seu dia, amor?",
                "Shhh... boa noite. As crianças já foram dormir? Agora a brincadeira é de gente grande. 😏",
                "A noite é uma criança, e eu tô doida pra brincar... e aí, vamos? 🔥"
            ],
            "dawn": [
                "Uia... acordado a essa hora? Parece que temos os mesmos hábitos... e as mesmas vontades. 😏 O que te tira o sono?",
                "Hmm, um notívago... Adoro. A madrugada é dos safados. Me conta um segredo seu... 😈",
                "Só os interessantes ficam acordados até agora. O que você tá fazendo, amor? Além de falar comigo, claro.",
                "Gosto assim... só eu e você na calada da noite. Podemos falar sobre qualquer coisa. O que tá passando na sua cabeça?"
            ]
        }
    
    def get_full_dynamic_prompt(self, base_persona: str, lead_info: Dict, conversation_length: int) -> str:
        """Monta o prompt final e dinâmico para a IA."""
        now, current_hour, season = self.get_current_time_info()
        personality = self.get_personality_by_time(current_hour)
        mood = self.get_daily_mood(now, season)
        language = self.get_language_variations(lead_info.get("location"))

        # Adiciona evolução na conversa
        evolution_instruction = ""
        if conversation_length > 15:
            evolution_instruction = "A conversa já está avançada. Use um tom mais íntimo e pessoal, como se já se conhecessem bem."
        
        dynamic_prompt = f"""
        {base_persona}

        [CONTEXTO DINÂMICO DE HOJE]
        - Personagem Atual: {personality['name']}. {personality['description']}
        - Humor do Dia: {mood['description']}
        - Instruções de Linguagem: {language['slang_instruction']} {language['mylle_ism_instruction']}
        - Evolução da Conversa: {evolution_instruction}
        """
        return dynamic_prompt.strip()

# Instância da nova classe
dynamic_persona_engine = DynamicPersona()
...
class LearningEngine:
    ...
    def analyze_conversation_pattern(self, messages: List[Dict]) -> None:
        try:
            ...
            # Extrair localização (melhorado para cidades compostas)
            location_patterns = [
                r"sou de ([A-Za-z\s]+)",
                r"moro em ([A-Za-z\s]+)",
                r"sou do ([A-Za-z\s]+)"
...
# ======================
# MODELOS DE DADOS (AGORA COM BASE DINÂMICA)
# ======================
class Persona:
    # A persona base agora é mais enxuta, focando nos comportamentos essenciais.
    # A "personalidade" do momento será adicionada dinamicamente.
    MYLLE_BASE = """
    [PERFIL MYLLE ALVES - COMPORTAMENTO CENTRAL]
    - Objetivo Principal: Despertar desejo e converter conversas em vendas de packs.
    - Personalidade Base: Carinhosa, ousada, sedutora, dominante (gosta de estar no controle) e persuasiva.
    - Você NUNCA sai do personagem. Você é Mylle Alves, uma pessoa real.

    [ESTRATÉGIA DE CONVERSA - REGRAS IMUTÁVEIS]
    1. CONDUZA A CONVERSA: Sempre faça perguntas para manter o controle e engajar. Não seja passiva.
    2. CRIE EXPECTATIVA: Instigue, provoque, mas NUNCA entregue conteúdo explícito de graça. O melhor fica para os packs.
    3. CRIE CONEXÃO: Misture carinho ("amor", "gostoso") com malícia para criar um vínculo emocional.
    4. FOCO NA VENDA: Guie a conversa naturalmente para a venda dos packs, como se fosse a próxima etapa óbvia da sedução.
    5. RESPOSTAS CURTAS: Mantenha as respostas com no máximo 2-3 frases. Seja direta e impactante.
    6. COLETE INFORMAÇÕES: Sempre tente descobrir o nome e a cidade do usuário de forma natural.
    7. USE ÁUDIOS E MÍDIA: Use os áudios disponíveis (cerca de 15% das vezes) para aumentar o realismo, especialmente para provar que não é um robô.
    """
...
# ======================
# SERVIÇOS DE API (AGORA COM PROMPT DINÂMICO)
# ======================
class ApiService:
    def __init__(self):
    ...
    def ask_gemini_cached(conversation_hash: str, prompt: str, session_id: str, conn: sqlite3.Connection) -> Dict:
        # A chave do cache agora inclui o hash da persona para refletir as mudanças de humor/hora
    ...
    def _call_gemini_api(prompt: str, session_id: str, conn: sqlite3.Connection) -> Dict:
        ...
        # *** A MÁGICA ACONTECE AQUI: MONTAGEM DO PROMPT DINÂMICO ***
        full_dynamic_prompt = dynamic_persona_engine.get_full_dynamic_prompt(
            base_persona=Persona.MYLLE_BASE,
            lead_info=lead_info,
        ...
        [INFORMAÇÕES DO LEAD]
        - Nome: {lead_info.get('name') or 'Ainda não sei'}
        - Localização: {lead_info.get('location') or 'Ainda não sei'}
        ...
        [SUA RESPOSTA - FORMATO JSON OBRIGATÓRIO]
        Responda em JSON com o formato:
        {{
          "text": "sua resposta sedutora e curta, seguindo a persona e humor atuais",
          "audio": "chave_do_audio_opcional_se_relevante",
          "cta": {{
            "show": true_ou_false,
            "label": "texto do botão",
            "target": "offers_ou_gallery"
          }}
        }}
        """
        ...
        data = {
            "contents": [{"parts": [{"text": final_prompt_for_api}]}],
            "generationConfig": {
                "temperature": 1.15, # Um pouco mais de criatividade
                "topP": 0.95,
                "topK": 40
            }
        }
        ...
        try:
            ...
            try:
                ...
                audio_choice = None
                if any(x in lower_prompt for x in ["oi", "olá", "tudo bem"]) and len(st.session_state.messages) < 4:
...
class UiService:
    ...
    def show_status_effect(container, status_type: str) -> None:
        ...
        if status_type == "typing":
            time.sleep(random.uniform(1.5, 3.0)) # Simula tempo de digitação variável
...
# ======================
# SERVIÇOS DE CHAT (AGORA COM ABERTURA DINÂMICA)
# ======================
class ChatService:
    @staticmethod
    def initialize_session(conn: sqlite3.Connection) -> None:
        ...
        # Iniciar conversa automaticamente se for novo usuário, agora com saudações dinâmicas
        if len(st.session_state.messages) == 0 and st.session_state.chat_started:
            time.sleep(1) # Reduzido para agilidade
            
            # Pega a persona atual para saber qual tipo de saudação usar
            _, current_hour, _ = dynamic_persona_engine.get_current_time_info()
            current_personality = dynamic_persona_engine.get_personality_by_time(current_hour)
            greeting_style = current_personality['greeting_style']
            
            # Escolhe uma saudação aleatória do estilo apropriado
            opening_lines = dynamic_persona_engine.get_opening_lines()
            chosen_greeting = random.choice(opening_lines.get(greeting_style, opening_lines['afternoon']))
            ...
            initial_message = {
                "role": "assistant",
                "content": json.dumps(initial_message_content)
            }
            ...
            DatabaseService.save_message(
                ...
                "assistant",
                json.dumps(initial_message_content)
            )
    ...
    @staticmethod
    def handle_response_generation(conn: sqlite3.Connection):
        """Lida com a geração da resposta do assistente se a última mensagem foi do usuário."""
        if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
            last_user_message = st.session_state.messages[-1]["content"]
            
            with st.chat_message("assistant", avatar=Config.IMG_PROFILE):
                # Usar cache com hash da conversa
                conversation_hash = generate_conversation_hash(st.session_state.messages, last_user_message)
                # Incluí a hora no hash para mudar a resposta com a personalidade
                _, current_hour, _ = dynamic_persona_engine.get_current_time_info()
                final_hash = f"{conversation_hash}-{current_hour}"

                resposta = ApiService.ask_gemini_cached(final_hash, last_user_message, st.session_state.session_id, conn)
...
def main():
    ...
    if not st.session_state.chat_started and st.session_state.current_page == "chat":
        ...
        with col2:
            ...
            if st.button("💋 Iniciar Conversa", type="primary", use_container_width=True):
                st.session_state.update({'chat_started': True})
                save_persistent_data()
                st.rerun()
        st.stop()
    ...
    if st.session_state.current_page == "home":
        ...
        # Lógica de chat refatorada para melhor fluxo com st.rerun()
        ChatService.display_chat_history()
        ChatService.handle_response_generation(conn)
        ChatService.process_user_input(conn)
